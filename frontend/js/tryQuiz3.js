function tryQuiz3(){
    $('#quiz3').bind('click',function(){
        $('#quiz3').attr('style','display:none')
        var qNumber = 0
        //$('#Content').html('')
        $('#Content3').append((
            "<!-- Button trigger modal -->"+
            "<button type='button' style='display:none' id = 'triggerQuiz3' class='btn btn-primary' data-toggle='modal' data-target='#exampleModal'>"+
            "  Launch demo modal"+
            "</button>"+
            ""+
            "<!-- Modal -->"+
...
"<div id = 'final3'></div>"+
            ...
            "        <button type='button' id = 'closeLevel3' class='btn btn-secondary'>Close</button>"+
            "        <button id = 'submitTest3' type='button' class='btn btn-primary'>Submit</button>"+
            "      </div>"+
            "    </div>"+
            "  </div>"
        ...
        $('#closeLevel3').bind('click',function(){
            $(location).attr('href','index.html')
        })
        $('#triggerQuiz3').click()
        
        $.ajax({
            url : 'http://127.0.0.1:5000/loadQ3',
            data : {},
            dataType : 'json',
            type : 'POST',
...
"</br>"
        ...
        $('#submitTest3').bind('click',function(){
            ...
            $.ajax({
                ...
                success : function(result){
                    ...
                    $('#final3').append("your mark for this quiz is "+mark+" %")
                    $('#submitTest3').attr('style','display:none')
                    $.ajax({
